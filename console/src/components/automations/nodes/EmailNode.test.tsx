...
// EmailNode only needs Handle/Position/useConnection from @xyflow/react; render them
// as inert so the node can mount without a ReactFlow provider.
...
// The reference list of templates comes from the automation context. It is populated
// by AutomationsPage's templates query; EmailNode resolves the selected template_id
// against it to display the template name.
...
describe('EmailNode', () => {
  it('shows the template name for a selected template of any category', () => {
    // Regression guard for the "Template set" bug: a welcome-category template is a
    // valid selection for an automation email node (welcome/unsubscribe emails are
    // sent via automations since v20), so as long as the reference list contains it,
    // the canvas must show its name — not the "Template set" fallback. This only holds
    // because AutomationsPage no longer restricts the reference list to 'marketing'.
