...
vi.mock('../components/automations/UpsertAutomationDrawer', () => ({ UpsertAutomationDrawer: () => null }))
...
describe('AutomationsPage template reference query', () => {
  ...
  it('fetches all email templates (no category filter) so email nodes resolve any selected template', async () => {
    // The automation email-node picker (EmailConfigForm -> TemplateSelectorInput) is
    // category-agnostic, so the canvas reference list must be too. Restricting it to
    // category:'marketing' made validly-selected non-marketing templates (e.g. a
    // welcome email) render as "Template set" instead of their name. This asserts the
    // reference query is not category-restricted — it fails on the pre-fix code that
    // passed category:'marketing'.
    renderPage()

    await waitFor(() => expect(templatesList).toHaveBeenCalled())
    const params = templatesList.mock.calls[0][0] as { workspace_id: string; channel?: string; category?: string }
